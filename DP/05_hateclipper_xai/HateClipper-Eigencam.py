...
model = CLIPClassifier.load_from_checkpoint("yourpath/HateClipper-CheckPoint-1103.ckpt")
model.eval()
...
def generate_eigen_cam(model, input_tensor, text_inputs, target_layer):
    ...
    class WrappedModel(torch.nn.Module):
        ...
        def forward(self, x):
            ...
            return logits
    wrapped = WrappedModel(model, text_inputs['input_ids'].to(device), text_inputs['attention_mask'].to(device)).to(device)

    #tokens to feature map
    def reshape_transform(tensor):
        # tensor: (batch, tokens, features)
        tensor = tensor[:, 1:, :]                     #del CLS
        batch, num_patches, features = tensor.shape
        side = int(np.sqrt(num_patches))              # 7 for ViT-B/32
        tensor = tensor.reshape(batch, side, side, features)
        tensor = tensor.permute(0, 3, 1, 2)           # (batch, features, side, side)
        return tensor

    cam = EigenCAM(model=wrapped, target_layers=[target_layer], reshape_transform=reshape_transform)
    grayscale_cam = cam(input_tensor=input_tensor)   # (1, side, side)
    cam_image = grayscale_cam[0]                      # (side, side)
    cam_resized = cv2.resize(cam_image, (224, 224), interpolation=cv2.INTER_CUBIC)
    return cam_resized
...
print("Eigencam:", all_scores.mean())
